...
async function whatsAsena () {
    ...
    conn.on('connecting', async () => {
        console.log(`${chalk.green.bold('Chintu')}${chalk.blue.bold('Bot')}
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp... Please Wait.')}`);
    ...
    conn.on('open', async () => {
        ...
        console.log(
            chalk.green.bold('𝙲𝙷𝙸𝙽𝚃𝚄 𝙱𝙾𝚃 v3.8.4 - Public Edition 💃♥️')
        );
        ...
        if (config.WORKTYPE == 'public') {
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                ...
                await conn.sendMessage(conn.user.jid, fs.readFileSync("./media/CyberQueen-alive.png"), MessageType.image, { caption: '*𝙲𝙷𝙸𝙽𝚃𝚄 𝙱𝙾𝚃 private ආකාරයට ක්‍රියා කරයි.*\n\n*Username:* ' + conn.user.name + '\n\n_මෙහි command උත්සාහ නොකරන්න. මෙය ඔබගේ ලොග් අංකය වේ._\n_ඔබට ඕනෑම චැට් එකක විධාන උත්සාහ කළ හැකිය :)_\n\n*ඔබේ command list එක ලබාගැනීමට .panel command බාවිතා කල හැකිය.*\n\n*ඔබේ bot private ක්‍රියාත්මක වේ. වෙනස් කිරීමට* _.var WORK_TYPE:public_ *විධානය භාවිතා කරන්න.*\n\n*᳆⃞🇱🇰ᴄʏͥʙᴇͣʀͫ↯ǫᴇᴇɴᬐ භාවිතා කිරීම ගැන ස්තූතියි.*', });
                }
            }
            else {
    ...
    conn.on('message-new', async msg => {
        ...
        if (config.SUPPORT1 == '120363021831502632@g.us') {     
                   ...
                   if ((config.OWN !== false && msg.key.fromMe === false && command.fromMe === true &&
                    ...
                    if (sendMsg) {
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                await conn.sendMessage(conn.user.jid, fs.readFileSync("./media/ERR.jpg"), MessageType.image, { caption: '*-- දෝෂ වාර්තාව [𝙲𝙷𝙸𝙽𝚃𝚄 𝙱𝙾𝚃] --*' + 
                                    ...
                                    '\nhttps://chat.whatsapp.com/EW96235JKZhLA2XAJsPrf1 ඔබට එය මෙම group යොමු කළ හැකිය._\n\n' +
                                    '*සිදු වූ දෝෂය:* ```' + error + '```\n\n'
                                    , });
                            } else {
                                await conn.sendMessage(conn.user.jid, fs.readFileSync("./media/ERR.jpg"), MessageType.image, { caption: '*-- දෝෂ වාර්තාව [𝙲𝙷𝙸𝙽𝚃𝚄 𝙱𝙾𝚃] --*' + 
                                    ...
                                    '\n_ඔබේ bot සඳහා යම් උදව්වක් අවශ්‍ය නම්, https://chat.whatsapp.com/EW96235JKZhLA2XAJsPrf1 වෙත පිවිසෙන්න...\n\n' +
                                    '*Report:* ```' + error + '```\n\n'
                                    , });
                            }
                        }
